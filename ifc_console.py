...
def process_ifc_query(ifc_path: str, question: str) -> str:
    ...
    similar_chunks = retrieve_similar_chunks(question, model, idx_path)
    answer = call_gemini_api(question, similar_chunks, GEMINI_API_KEY)
    return answer

def main():
    print("=== IFC Console Application ===")
    print("Podaj ścieżkę do pliku IFC lub naciśnij Enter dla domyślnego (default.ifc):")
    
    ifc_path = input().strip()
    if not ifc_path:
        ifc_path = "default.ifc"
    
    if not os.path.exists(ifc_path):
        print(f"Plik {ifc_path} nie istnieje!")
        return
    
    print(f"Załadowano plik IFC: {ifc_path}")
    print("Możesz teraz zadawać pytania o model IFC. Wpisz 'exit' aby zakończyć.")
    print("Przykłady pytań:")
    print("- ile ścian")
    print("- how many doors")
    print("- powierzchnia ścian")
    print("- ile kondygnacji")
    print()
    
    while True:
        question = input("Pytanie: ").strip()
        
        if question.lower() in ['exit', 'quit', 'wyjście', 'koniec']:
            print("Do widzenia!")
            break
